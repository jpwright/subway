class Line {
    ...
    insert_station_at_pos(station_id, line_insertion_pos) {
        ...
        if (line_insertion_pos < this.stations.length && this.stations.length > 1) {
            neighbors.push(this.stations[line_insertion_pos]);
            var edge_stations_next = [this.stations[line_insertion_pos], station_id];
            var edge_next = new Edge(edge_stations_next, this.id);
            N_edges[edge_next.id] = edge_next;
            
            $.ajax({ url: "edge-add?service-id="+NS_service_id.toString()+"&line-id="+this.NS_id.toString()+"&station-id-1="+N_stations[edge_stations_next[0]].NS_id.toString()+"&station-id-2="+N_stations[edge_stations_next[1]].NS_id.toString(),
                async: true,
                dataType: 'json',
                success: function(data, status) {
                    N_edges[edge_next.id].NS_id = data["id"];
                }
            });
            
        }
        if (line_insertion_pos > 0 || this.stations.length == 1) {
            neighbors.push(this.stations[line_insertion_pos-1]);
            var edge_stations_prev = [this.stations[line_insertion_pos], station_id];
            var edge_prev = new Edge(edge_stations_prev, this.id);
            N_edges[edge_prev.id] = edge_prev;
            
            $.ajax({ url: "edge-add?service-id="+NS_service_id.toString()+"&line-id="+this.NS_id.toString()+"&stop-id-1="+N_stations[edge_stations_prev[0]].NS_id.toString()+"&stop-id-2="+N_stations[edge_stations_prev[1]].NS_id.toString(),
                async: true,
                dataType: 'json',
                success: function(data, status) {
                    N_edges[edge_prev.id].NS_id = data["id"];
                }
            });
        }
        
        if (neighbors.length == 2) {
            // Delete the old edge connecting the neighbors
            for (var i = 0; i < N_edges.length; i++) {
                var edge = N_edges[i];
                var stations = edge.stations;
                
                if ((stations[0] == neighbors[0] && stations[1] == neighbors[1]) ||
                    (stations[1] == neighbors[0] && stations[0] == neighbors[1])
                ) {
                    N_edges[i].active = false;
                }
            }
        }
        ...
        if (USE_CURVED_TRACKS) {
            ...
            if (this.draw_map.length > 1) {
                ...
                for (i = 1; i < this.draw_map.length; i++) {
                    ...
                    //var control_points = [control_point_1, control_point_2];
                    //var edge = new Edge([station_prev.id, station_next.id], track, control_points);
