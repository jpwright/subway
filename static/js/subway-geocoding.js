class Geocoder {
    ...
    geocode(line) {
        ...
        */
        ...
        $.ajax({ url: "station-add?service-id="+NS_service_id.toString()+"&lat="+this.latlng.lat+"&lng="+this.latlng.lng, 
            async: false,
            dataType: 'json',
            success: function(data, status) {
                ...
                var neighborhood = "";
                if ("locality" in data) {
                    geo.info = data["locality"];
                }
                
                if ("neighborhood" in data) {
                    neighborhood = data["neighborhood"];
                    geo.info += "<br />"+neighborhood;
                }
                
                var station_id = geocode_to_station(geo, line, borough, neighborhood);
                N_stations[station_id].NS_id = data["id"];
                station = N_stations[station_id];
            }
        });
        $.ajax({ url: "stop-add?service-id="+NS_service_id.toString()+"&line-id="+line.NS_id.toString()+"&station-id="+station.NS_id.toString(),
            async: true,
            dataType: 'json',
            success: function(data, status) {
            }
